...
public class RawDataPlot {
    ...
    public RawDataPlot(Collection<Vector> vectors, double diameter, double majorTickSpread, double minorTickSpread) {
        super();
    ...
    public RawDataPlot(Collection<Vector> vectors, double diameter) {
        super();
        this.vectors = vectors;
        this.diameter = diameter;
        this.majorTickSpread = Math.PI / 6;
        this.minorTickSpread = Math.PI / 12;
    }

    public RawDataPlot(Collection<Vector> vectors) {
        super();
        this.vectors = vectors;
        this.diameter = 1024;
        this.majorTickSpread = Math.PI / 6;
    ...
    public SVGDocument draw() {
        ...
        Map<Integer, List<Vector>> observations = new TreeMap<>();

        for (Vector vector : vectors) {
            double degrees = vector.getDegrees360();
            int index = (int) degrees;

            if (!observations.containsKey(index)) {
                observations.put(index, new ArrayList<Vector>());
            }

            List<Vector> list = observations.get(index);
            list.add(vector);
        }

        double observationSize = 0.025 * radius;

        for (Entry<Integer, List<Vector>> entry : observations.entrySet()) {
            ...
            for (Vector vector : entry.getValue()) {
                ...
                if (degree == 90 || degree == 270) {
                    ...
                    // 'ap', 'bp' = equation for perpendicular line to 'a', 'b'
                    double ap = -a;
                    double bp = yv + a * xv;
        ...
        for (String line : lines) {
            ...
            for (String token : StringUtils.split(line)) {
                if (!StringUtils.isBlank(token)) {
                    vectors.add(Vector.fromHourMinuteString(token));
                }
            }
        }

        RawDataPlot plot = new RawDataPlot(vectors);
