...
public class DynamicProgrammingAll extends AbstractDynamicProgramming {
   ...
   *     heuristic to remove single regions prior to a
   */
  public DynamicProgrammingAll(final RegionRemover regionRemover, final int maxCliqueSize) {
  ...
  public final SubSolution[] findOptimalSolutions(final Clique clique) {
    ...
    for (int j = 1; j < size; j++) {
      for (int i = j - 1; i >= 0; i--) {
        matrix[i][j] = AbstractDynamicProgramming.solveSingleCase(matrix, clique, i, j);
      }
    }
