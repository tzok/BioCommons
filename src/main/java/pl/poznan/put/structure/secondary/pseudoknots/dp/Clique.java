...
/**
 * A collection of regions conflicting with each other. Precisely, this is not a
 * real clique. Each region in this collection has conflict with *at least one*
 * other region. It does not mean that every region conflicts with every other!
 */
public class Clique {
    private final MultiKeyMap<Integer, Region> beginEndRegion =
            new MultiKeyMap<>();
    ...
    public final String toString() {
        return String.format("Clique{firstEndpoint=%d, lastEndpoint=%d, "
                             + "regionCount=%d}", sortedEndpoints.get(0),
                             sortedEndpoints.get(sortedEndpoints.size() - 1),
                             regions.size());
    }
}
